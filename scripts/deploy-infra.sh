...
if resource_exists "Microsoft.Storage/storageAccounts" "${FUNC_SA_NAME}"; then
  echo "  ✓ Storage Account (${FUNC_SA_NAME}) exists"
  FUNC_SA_EXISTS=true
else
  echo "  ✗ Storage Account (${FUNC_SA_NAME}) missing"
fi
...
if [[ "${1:-}" == "--what-if" ]]; then
  ...
  if [[ "${FUNC_SA_EXISTS}" == "false" ]] || [[ "${FUNC_EXISTS}" == "false" ]] || [[ "${FORCE}" == "true" ]]; then
    echo "▶ Deploying Function App (Storage Account + Consumption Plan + Function App)..."
    az deployment group create \
      --resource-group "${RESOURCE_GROUP}" \
      --name "fedex-func-$(date +%Y%m%d-%H%M%S)" \
      --template-file "${REPO_ROOT}/infra/function-app.json" \
      --parameters \
